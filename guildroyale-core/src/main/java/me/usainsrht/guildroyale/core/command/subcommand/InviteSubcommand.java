...
/** {@code /guild invite <player>} */
@SuppressWarnings("UnstableApiUsage")
public final class InviteSubcommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx) {
        ...
        plugin.getScheduler().runAsync(() ->
                plugin.getGuildService().getGuildByMember(player.getUniqueId()).thenCompose(opt -> {
                    ...
                    return plugin.getMemberService().invite(opt.get().getId(), player.getUniqueId(), target.getUniqueId())
                            .thenAccept(result -> plugin.getScheduler().runForEntity(player, () -> {
                                switch (result) {
                                    case ActionResult.Success s -> {
                                        player.sendMessage(plugin.getMessages().prefixed("invite-sent",
                                                Placeholder.unparsed("player", targetName)));
                                        plugin.getScheduler().runForEntity(target, () ->
                                                target.sendMessage(plugin.getMessages().prefixed("invite-received",
