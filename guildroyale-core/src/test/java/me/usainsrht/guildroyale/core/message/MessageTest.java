...
class MessageTest {
    ...
    void testSingleStringSoundParsing() {
        // Sound name only
        SoundData entry1 = SoundData.fromString("entity.player.levelup");
        assertEquals("entity.player.levelup", entry1.key());
        assertEquals(1.0f, entry1.volume());
        assertEquals(1.0f, entry1.pitch());
        assertEquals(Sound.Source.MASTER, entry1.source());

        // Sound name + volume
        SoundData entry2 = SoundData.fromString("entity.player.levelup, 0.5");
        assertEquals("entity.player.levelup", entry2.key());
        ...
        SoundData entry3 = SoundData.fromString("entity.player.levelup, 1.5, 2.0");
        assertEquals("entity.player.levelup", entry3.key());
        ...
        SoundData entry4 = SoundData.fromString("entity.player.levelup,1,2,master");
        assertEquals("entity.player.levelup", entry4.key());
        ...
        SoundData entry5 = SoundData.fromString("entity.player.levelup, 0.8, 1.2, player");
        assertEquals("entity.player.levelup", entry5.key());
    ...
    void testMessageParseMapWithCommaSeparatedSound() {
        ...
        YamlMessage message = YamlMessage.parse(map);
        assertNotNull(message.sounds());
        assertEquals(1, message.sounds().size());

        SoundData sound = message.sounds().get(0);
        assertEquals("entity.player.levelup", sound.key());
    ...
    void testMessageParseMapWithSoundList() {
        ...
        SoundData s1 = message.sounds().get(0);
        assertEquals("entity.player.levelup", s1.key());
        assertEquals(1.0f, s1.volume());
        assertEquals(2.0f, s1.pitch());
        assertEquals(Sound.Source.MASTER, s1.source());

        SoundData s2 = message.sounds().get(1);
        assertEquals("entity.experience_orb.pickup", s2.key());
    ...
    void testSoundKeyWithUnderscore() {
        ...
        YamlMessage message = YamlMessage.parse(Map.of("sound", "entity.ender_dragon.death"));
